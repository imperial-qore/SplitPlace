...
def setupVLANEnvironment(cfg, mode):
    ...
    uname = "ansible"
    for ip in HOST_IPS:
        res = os.system("ssh -o StrictHostKeyChecking=no -i workflow/install_scripts/ssh_keys/id_rsa "+uname+"@"+ip+" ~/agent/scripts/delete.sh > /dev/null 2>&1")  
        ...
        res = os.system("ssh -o StrictHostKeyChecking=no -i workflow/install_scripts/ssh_keys/id_rsa "+uname+"@"+ip+" sudo rm -rf ~/container_data/* > /dev/null 2>&1")  
    return HOST_IPS
